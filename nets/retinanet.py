...
class GFocalRegHead(nn.Module):
    def __init__(self,
        ...
        self.num_anchors=num_anchors
        self.num_output=4*(reg_max+1)   # = num of delta y
    ...
    def forward(self,x):
        x=self.reg(x)
        bs,_,h,w=x.shape
        x=x.permute(0,2,3,1).contiguous().view(bs,h,w,self.num_anchors,self.num_output).view(bs,-1,self.num_output)
        return x
...
class GFocalHead(nn.Module):
    def __init__(self,in_channel,
        ...
        self.cls_bones=SequenceCNR(in_channel,inner_channel,kernel_size=3,stride=1,num=num_convs,block_type=block_type)
        self.reg_bones=SequenceCNR(in_channel,inner_channel,kernel_size=3,stride=1,num=num_convs,block_type=block_type)
    ...
    def forward(self,xs):
        ...
        if self.training:
            ...
            for cls_out, reg_out, stride, anchor in zip(cls_outputs, reg_outputs, self.strides, self.anchors):
                ...
                reg_out=self.project(reg_out)*stride  # shape=[bs,-1,4] 4==>l,t,r,b
...
class GFocal(nn.Module):
    ...
    def forward(self,x,targets=None):
        ...
        if self.training:
            ...
            loss_qfl, loss_iou, loss_dfl, num_pos = self.loss(cls_outputs, reg_outputs, anchors, targets)
